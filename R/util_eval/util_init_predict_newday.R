run_predict_reward_side <- function(name, a = dmodel, a_model = result_model){
  ...
  for(i in seq_along(mice)){
    ...
    dmouse = a %>%
      filter(tag %in% mouse) %>%
      arrange(start)
    dmouse$prob0 = predictC(mouse_par, dmouse$dooropened, dmouse$side, dmouse$session)
...
run_predict_reward_side_interval <- function(name, a = dmodel, a_model = result_model){
  ...
  for(i in seq_along(mice)){
    ...
    dmouse = a %>%
      filter(tag %in% mouse) %>%
      arrange(start)
    dmouse$prob0 = predictC(mouse_par, dmouse$dooropened, dmouse$side, dmouse$interval, dmouse$session)
    out[[i]] <- (dmouse %>% mutate(probchosen = ifelse(side %in% 0, prob0, 1-prob0)))}
  return(dplyr::bind_rows(out))}
